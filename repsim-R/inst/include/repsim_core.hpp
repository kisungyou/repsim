...
namespace repsim {
  ...
  inline Mat util_ONB(const Mat& X){
    ...
    if (r == 0) return Mat::Zero(X.rows(), 0);
  ...
  inline double util_upper_median(const Mat& D){
    ...
    for (Index i=0; i<(n-1); i++){ // assign process
  ...
  inline Mat kernel_rbf_dualmed(const Mat& X){
    ...
    std::vector<double> row_medians; row_medians.reserve(N);
    for (Index i=0; i<N; i++){
      std::vector<double> row_vals; row_vals.reserve(N-1);
      for (Index j=0; j<N; j++){
        if (i != j){
	...
	// kernel_select : iteratively modify ---------------------------- UPDATE
  ...
  inline Mat core_HSIC(const std::vector<Mat>& Xs, 
    ...
    std::vector<Mat> batch_K; batch_K.reserve(M);
    for (Index i=0; i<M; i++){
  ...
  inline Mat core_CKA(const std::vector<Mat>& Xs, 
    ...
    // Precompute kernels
    std::vector<Mat> K; K.reserve(M);
    for (Index i = 0; i < M; ++i) K.emplace_back(kernel_select(Xs[i], kernel_type));
  ...
  inline Mat core_CCA(const std::vector<Mat>& Xs, 
    ...
    std::vector<Mat> vec_Q; vec_Q.reserve(M); // batch compute ONBs
    std::vector<double> vec_dims; vec_dims.reserve(M);
    for (Index m=0; m<M; m++){
  ...
  inline double core_PWCCA_pair(const Mat& X, const Mat& Y) {
    ...
    Index rX = 0, rY = 0;
    for (Index i = 0; i < sX.size(); ++i) if (sX[i] > tX) ++rX;
    for (Index i = 0; i < sY.size(); ++i) if (sY[i] > tY) ++rY;
    ...
    if (wsum <= 0.0 || !std::isfinite(wsum)) {
      ...
      w.setOnes(); wsum = static_cast<double>(r);
    }
    w.array() /= wsum;
